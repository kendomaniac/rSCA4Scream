...
def get_mnist():
    ...
    return Atac, Atac
X, Y  = get_mnist()
c = DeepEmbeddingClustering(n_clusters=5, input_dim=X.shape[1],Atac=X,bias=bias,bName=bName,sep=sep,path=path)
c.initialize(X, finetune_iters=2, layerwise_pretrain_iters=2)
c.cluster(X, y=Y)
